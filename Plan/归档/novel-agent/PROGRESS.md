...
| P9 出图 | [x] | PROPOSED 落盘 | schema v6 `generated_images` + `NovelImageStore` 队列 + UI 预览/CANON + checklist；`imagegen:ok` |
| P10 MCP | [~] | 只读 tools/list+call | HTTP+SSE+stdio 完成；Redis/Garnet：App 已 `db::Init`；待本机 Garnet 实测 cache 命中 |
...
| ID | 任务 | 状态 | 备注 |
|----|------|------|------|
| P9.1 | ImageBackend + 设置 | [x] | `NovelImageGen` + Settings `imageBackend/BaseUrl/ApiKey/Model/WxH/Steps`；mock/openai_images/comfy 占位；设置窗「出图」段；`SHINE_NOVEL_GRAPH_CHECK` imagegen:ok |
| P9.2 | 队列落盘 PROPOSED | [x] | schema v6 `generated_images`；`RunImageJob` Assemble→Generate→PROPOSED/FAILED；`visual/gen/<job_id>.png`；队列自检 PASS |
| P9.3 | UI 预览 + 标 CANON | [x] | NovelView「出图」段：生成/列表/缩略图/标 CANON·NON_CANON·DISCARDED；worker 解码 + UI 上传纹理 |
| P9.4 | （可选）Critic/checklist | [x] | `EvaluateImageChecklist` 启发式（prompt/落盘/尺寸/多人/negative）写入 `checklist_json` |

## P10 · MCP

| ID | 任务 | 状态 | 备注 |
|----|------|------|------|
| P10.1 | stdio Server 骨架 | [x] | `RunStdioServerMain` + `ShineTVStudio.exe --mcp-stdio`；stdout 仅 JSON-RPC |
| P10.2 | 只读 tools 接 NovelService | [x] | `novel_get_relations/chapter/foreshadows/secrets_for/event_chain/ownership/slice/memory` 等与 P4 对齐 |
| P10.3 | HTTP/SSE remote | [x] | `POST /mcp` JSON + `Accept: text/event-stream` streamable；`GET /sse` 长连接 endpoint/session/ping；`McpHttpClient` 自检 |
| P10.4 | Redis 可选缓存/锁 | [~] | App `db::Init`；Garnet 本机 6379 运行中（`E:\garnet\main\GarnetServer`，`dotnet run -c Release -f net10.0`） |
| P10.5 | 写 tools + 开关 + audit | [x] | Settings `mcpAllowWrite` + UI + `audit_logs`；默认拒写 PROPOSED |
| P10.6 | 客户端配置文档 | [x] | `P10-mcp/CLIENT.md`（HTTP + stdio + Claude 片段） |
...
| 项 | 说明 | 状态 |
|----|------|------|
| OPENAI/MiMo/MiniMax Key | Chat / Responses / **Anthropic Messages** 均已接；`LlmComplete` 按 `llmProtocol` 分流；离线自检过 | [~] 待配 Key 真调用 |
| Garnet/Redis | 已 `db::Init`；**本机 6379 在听**（GarnetServer PID 31312，`E:\garnet\main\GarnetServer`） | [~] 再跑一次 MCP 读实体看 cache 日志即可勾 x |
| 图片后端 | mock + openai_images 可用；Comfy 仍为占位 | [~] Comfy 可选 |
| MCP 实联调 | 本地 HTTP/SSE 自检 PASS；外部 Agent/MiMo 需配 Key 后跑 `RunMcpBridgedAgent` 或手测 `/mcp` | [~] 待 Key |
