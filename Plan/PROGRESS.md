...
**待办（未做）**：① K09 的 `shots` 起止状态列（`12` §1.4 缺口）与 `Beat[]`/`PromptArtifact` 的承载表 —— 这三组目前是 `contract-input`，传空即 `n/a`；② 章内**重试**产出阶段（`03` §2.6 `max_validate_retry=2`）：现在 S1 的「2 次」靠章内多次校验/多次观测累积，还没有"K 校验失败 → 回 `STATE_EXTRACT` 重做"的自动回路。
