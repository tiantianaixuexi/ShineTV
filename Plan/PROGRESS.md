...
- [x] **S37** **按镜下发的紧凑化**（解决"靠后的镜看不到上游设计"）— `s37-per-shot-stage-design`｜起因（S36 记的隐患）：V9 **按文件**下发 V2–V7 产物（每个截 3000 字）⇒ 镜一多，**靠后的镜根本没看到**设计 ⇒ S34 报的"阶段偏离"对它们**不是"没采纳"而是"没看到"**（冤枉 LLM）｜改法：**逐镜紧凑对齐** —— 每镜一小段、六阶段并排（键 `(scene_ord, ord)`），每镜都看到**属于自己的**设计；索引提到 prompt 构造之前（三处共用：下发 / 落库 / 比对）；超 **12000** 字才截且**截断进 `warnings`**（不静默）；旧 S32 块**删除**；`prompt_rule_version` **4→5**；**自检新增正反断言**（含"逐镜设计"段 + V2 设计按镜出现 + 按 `scene_ord/ord` 分条 + **旧「按文件下发」形态必须消失**）；**真跑**：`新写 6/复用 0`（规则版本生效）、K 校验无缺列；**25 项全 ok**｜**顺带**：把「为什么还有这么多盘上产物」的**架构判断**写进 `01`（SQLite 是唯一权威；V3–V7 已属世界状态却只在盘上 = 历史偏差；下一步该落库 `stage_artifacts`，**需拍板**）
- [x] **S36** **给 V2 一个承载：`shots.intent_json`**（**schema v11**）— `s36-shot-intent-field`｜用户拍板"给 NarrativeShot 加 intent 字段"（原先记的是"V2 的七问无处可落、无从比对"）｜① **落库侧**：`shots.intent_json`（建表直带 + `AddShotIntentColumn` 旧库补列 + `Migrate`/`EnsureSchemaUpToDate`/自检都调）、`ShotRow.intent_json` + `UpsertShot`（INSERT/UPDATE）/`GetShot`/`ListShotsByChapter` 四处读写；② **V9 落库**：从 V2 产物 `v02_director_intent.json` 按 `(scene_ord, ord)` 取（**七问是 V2 的职责，不在 V9 的 LLM 输出里**），没跑 V2 → `{}`；③ **V11 桥**：`NarrativeShotInput.intentJson`（**只承载、不参与计算** —— 桥仍是纯函数），`NovelGeneration` 从 `ShotRow` 带过去；④ 顺带：`JsonText` 参数收紧为 `const`（yyjson 签名没带 const）；⑤ **诚实记账**：V9 下发阶段产物**截断到 3000 字** ⇒ 靠后的镜可能没看到设计 → S34 的"阶段偏离"可能是"没看到"而非"没采纳"，现在截断进 `warnings`；**真跑**：`--novel-prompt 1` 复用 6（迁移生效）、`--novel-checks 1 --all` 无缺列错误；**25 项全 ok**
- [x] **S35** **空间层单一来源**（V10 改读 V4 产物）+ **V9 骨架硬校验** + **`--up-to`** — `s35-spatial-single-source`｜① **消灭"同一个概念两条路"**：V10 原读 `storyboard.json` 的 `spatial`（那是 V9 把 **V4 的结果又抄了一遍**）→ 改读 **V4 产物** `v04_spatial.json`（`items[]` 每条本身就是 spatial）；没跑 V4 → 该层空、**不阻断**；⇒ `prompt_rule_version` **3→4**；② **骨架硬校验**（`StoryboardRequest.strict_skeleton`，CLI `--strict`）：线骨架此前只是"下发给 LLM 的强建议"，现在三类偏离（缺失/新增/时长差>0.2s）**直接判失败** —— 把"强建议"变成"**合同**"；⚠️ **默认关**（真实工程常没跑 V1，硬开会让 V9 全线失败）；③ **`--up-to Vn`**：只跑到某阶段（调试省 LLM 调用，如 `--novel-stages 1 --up-to V4` 拿下空间层即可），非法值**在查 Key 之前**就报错；**真跑**：`--up-to 乱写` → 立刻 `不合法`；`--novel-prompt 1` → 规则版本变 ⇒ `新写 6/复用 0`，再跑 `复用 6`；**25 项全 ok**
- [x] **S34** **阶段产物的字段级比对**（把 S33 的"可检测"从骨架扩到各阶段设计）+ **LLM 前置检查** — `s34-preflight-and-stage-fields`｜① `StoryboardOutcome` 加 `stage_mismatch`：V9 逐镜比对**每阶段一个有代表性的字段** —— **V4 前景**（`spatial.layers.foreground`）/ **V5 景别**（`camera.shot_size`）/ **V3 表情**（`performance.expression`）/ **V7 环境音**（`audio.ambient`），偏离**全部进 `warnings`**（只列前 3 条 + 汇总，避免刷屏）；⚠️ 只挑一个字段是**刻意的**（全字段会被措辞差异淹没，要抓的是"根本没采纳"）；任一边为空**不判**；V2 仍无法比对（无承载字段）；② **LLM 前置检查**：`--novel-stages` / `--novel-storyboard` 在**调用之前**检查 Key，报"没配 API Key + 去哪配 + 这不是链路 bug 是缺外部凭据"；V10/V11/V8/K 校验不调 LLM 故不检查；**真跑**：前置检查 `ExitCode=2`、**耗时 0.1s/0s**（改前要等一次网络调用）；自检新增断言（V3 说"平静"而 mock 输出"警惕" ⇒ `stage_mismatch>0` 且进 warnings）；**25 项全 ok**
- [x] **S33** **软约束的"可检测"化**：V9 逐镜比对 V1 骨架（回答"LLM 会不会乱来"= **不靠猜、靠查**）— `s33-v9-consistency-check`｜`StoryboardOutcome` 新增 `skeleton_total / missing / extra / duration_mismatch`；V9 解析端逐镜比对 **骨架内缺失 / 骨架外新增 / `duration` 差 > 0.2s**，三类**全部进 `warnings` + 一条汇总 `log::Info`**（⚠️ **只记账不 fail** —— LLM 合并/拆镜有时合理，`03` §2.2 本就允许合并；但**绝不静默**）；**实测**（`SHINE_NOVEL_GRAPH_CHECK`）：一致路径 `下发 2 镜 / 缺失 0 / 新增 0 / 时长偏离 0`、**不一致路径 `下发 3 镜 / 缺失 1`**（骨架 3 镜而输出 2 镜 ⇒ 抓到）；自检新增两条断言（一致→三类为 0；不一致→ `missing=1` 且进 warnings）；**25 项全 ok**；⚠️ **附带发现**：**V2 的产物无法这样查** —— 它的七问 + `intensity` 在 `NarrativeShot` **没有承载字段**（V3–V7 都有）⇒ 只能影响判断、落不了地 —— 如实记账
- [x] **S32** **V2–V7 全部独立成阶段**（影视化链阶段化完成）+ **链式哈希** — `s32-v2-v7-stages`｜`NovelVisualStages` 扩成"**一张规格表 + 一个通用执行器**"（6 个阶段同构：读上游 → 一次 LLM（`Planner` 中档）→ 解析 `items[]` → 落盘，**不抄六遍**）：V2 `DIRECTOR_INTENT`（`12` §2.3 七问 + `intensity`，E1–E3 写进指令）/ V3 `PERFORMANCE`（12 项）/ **V4 `SPATIAL`**（`layers{foreground,midground,background}` = "谁在前景"的唯一来源，**V10 第 5 层消费它**）/ V5 `CAMERA`（10 项 + 枚举）/ V6 `TIMELINE`（`Beat[]`）/ V7 `AUDIO`（`13` §2.4 五层）；🔴 **链式哈希**：本阶段 hash = 库状态哈希 ⊕ **上游产物 hash** ⇒ **上游变了下游必失效重算**（否则改 V4 后 V5 还复用旧结果）；`RunAllVisualStages` 一次跑 V1→V7（哈希复用自动跳过已跑的）；⚠️ **全部被 V9 消费**（V1 骨架定切分 + V2–V7 原样下发进 prompt，软约束 + warnings）；CLI `--novel-stages` 改为跑全链；**真跑**：链路通（走到 LLM 调用，缺 Key 给可读错误）；自检 **25 项全 ok**（新增：V2 读 V1 的 `upstream_hash == V1 的 input_state_hash`、V2 复用不得再调 LLM）；🔴 又踩 raw string：`R"-…"` 漏开界符 `(` + 批量替换误改 `R"-((`
- [x] **S31** **V1 `SCENE_BREAKDOWN` 独立成阶段**（影视化链阶段化的第一步）+ V9 消费镜骨架 — `s31-v1-scene-breakdown`｜新增 `src/agent/NovelVisualStages.*`（登记 CMake；App 自检 **24→25 项**）：产出 `12` §2.2 的**七要素** + **镜骨架**（每场几镜 / 每镜 `ord`/`duration`/概要）→ `work/ch<NNN>/v01_scene_breakdown.json`；**`conflict` 为空 → 告警**（`12` §2.2 的硬校验，不静默放过）；带 `input_state_hash` ⇒ 哈希一致**复用、不重调 LLM**（阶段级续跑）；⚠️ **产物立刻被消费**：`NovelStoryboard`（V9）读镜骨架下发进 prompt ⇒ **"镜的切分"从 V9 提前到 V1**（软约束 + `warnings` 可见）——**避免"产出一份没人用的中间文件"**；CLI `--novel-stages <ch>`；**真跑**：链路通（走到 LLM 调用）、因无 API Key 给出可读错误（与正文链同一外部依赖）；自检 **25 项全 ok**（含"哈希复用不得再调 LLM""缺 conflict 必须告警""骨架可读回"）；🔴 顺带踩到 **C++ raw string 的 `")"` 提前终止**坑（多段拼接的 JSON 非法）——改用 `)-"` 分隔符单段；**后续顺序**：V2 → V3 → **V4（已被 V10 消费，优先级高）** → V5 → V6 → V7
- [x] **S30** **V8 `CONTINUITY` 独立成阶段**（`12` §3 的 **12-6 标 S 级**收口）+ `--novel-continuity` CLI — `s30-visual-stages`｜新增 `src/novel/NovelContinuity.*`（登记 CMake；App 自检 **23→24 项**）：判**相邻两镜**的「本镜 `end_state` → 下一镜 `start_state`」（`12` §2.7 的 **C1–C12**：角色集合 / 位置 / 朝向 / 姿势 / 手部 / 服装 / 伤势 / 道具持有与状态 / 光 / 环境 / 机位）；数据两处（`shots` 起止状态 + `work/ch<NNN>/storyboard.json` 的 `performance`/`spatial`/`camera`/`transition`）；**三态**（`pass` / `fail` / **`unverified`** —— C6/C7/C9/C11 需 `ChapterPlan.events` 或语义判断，**不假装通过**）；报告落 `work/ch<NNN>/v08_continuity.json`（**此前完全不存在**）；**真跑**：`--novel-continuity 1` → `6 镜 / 5 对相邻镜：违规 0 / 无法核对 0` + 报告落盘；自检覆盖"C4 姿势变而表演层空 → 违规""C12 start_state.camera_position 与故事板 camera.position 不一致 → 违规""无故事板 → unverified 不误报"；**24 项全 ok**｜⚠️ **V1–V7 仍按 `03` §2.2 允许的"合并"执行**（V9 一次推演）—— 逐个独立是合法但非必需的粒度提升（标 B）
- [x] **S29** **Comfy 出图后端的真实实现**（G13 的 stub 替换）— `s29-comfy-backend`｜`ComfyStubBackend`（恒返回"将在 P9.2 接入"）→ **`ComfyBackend`**：`video::BuildSceneToImageWorkflow` 编 SD1.5 workflow（**唯一来源**，本层不重写规则）→ `POST /prompt` → 轮询 `GET /history/{id}`（500ms × **300s 有界**）→ `GET /view` 下载 → 写盘；**同步短事务**（一次调用一提交，**headless CLI 可用**，不依赖 GUI 的常驻 `ComfySession`）；种子由 prompt/尺寸/steps 派生（`11` §2.5 **禁纯随机**）；错误全中文可读、轮询中瞬时错误不判死、首版**不上传参考图**（走纯文生图 + 记降级，账里说明）；**真跑**：`--novel-generate-images 1 --force`（工程配置即 `comfy`）→ 6 镜全部报**真实链路**的中文提示「工程缺少分镜图「Checkpoint」…」——**不再是 stub 文案**（证明已走进编排 workflow 那一步）、单镜不阻断
